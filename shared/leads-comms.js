...
(function (global) {
  function _authH() {
    ...
    var t = ''; try { t = localStorage.getItem('cc_token') || ''; } catch(e){}
  ...
  function _jsonAuthH(){
    return Object.assign({'Content-Type':'application/json'}, _authH());
  }
  ...
  function refreshTeam(){
    return fetch('/api/users', { headers: _authH() })
      .then(function(r){ if(r.status===401) return []; return r.json(); })
      .then(function(data){
  ...
  function addTeamMember(m){
    ...
    fetch('/api/users', {
      method: 'POST', headers: _jsonAuthH(),
